...
export async function buildMembershipAgreementPdfBytes(
  ...
  const documentTitle =
    input.documentTitle ?? "Kingston Jiu Jitsu Membership Agreement";
  const maxWidth = PAGE_WIDTH - MARGIN_X * 2;

  writer.drawLines(wrapText("KINGSTON JIU JITSU", maxWidth, bold, TITLE_SIZE), {
    font: bold,
    size: TITLE_SIZE,
  });
  ...
  for (const section of sections) {
    ...
    for (const paragraph of section.paragraphs) {
      if (
        !section.title &&
        (paragraph === "KINGSTON JIU JITSU" ||
          paragraph === "MEMBERSHIP AGREEMENT" ||
