...
interface MessageInstructorsPageProps {
  params: { clubSlug: string };
}

export async function generateMetadata({
  params,
}: MessageInstructorsPageProps): Promise<Metadata> {
...
export default async function MessageInstructorsPage({
  params,
}: MessageInstructorsPageProps) {
  const club = await requireClubBySlug(params.clubSlug);

  return (
    <main className="mx-auto min-h-screen w-full max-w-6xl space-y-6 px-3 py-4 pb-20 sm:px-5">
      <AppHeader pageTitle="Message Instructors" clubName={club.name} />
      ...
      <section className="rounded-xl border border-dojo-border bg-dojo-surface p-4">
        <AcademyMessagingTool
          ...
          description="Send messages to Instructor Portal users."
        />
      </section>
    </main>
  );
